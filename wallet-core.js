// Priscion MUSE Wallet Core v14.0.0
// THE SOVEREIGN OS: NO SIMULATIONS | FULL INTERACTIVITY | WHATSAPP FIDELITY
...
var lynxChats = [
    { handle: 'Priscion', lastMsg: 'Architect, the ledger is secure.', time: '21:10', avatar: 'P', unread: 1, type: 'architect' },
...
async function renderWallet() {
    ...
    c.innerHTML = `
        <div style="height:100%; display:flex; flex-direction:column; font-family:'Inter', sans-serif; background:#FFF; color:#1A1A1A;">
            <!-- SOVEREIGN TOP HEADER (VESPA STYLE) -->
            <div style="padding:15px 20px; border-bottom:1px solid #EEE; display:flex; justify-content:space-between; align-items:center; background:#FFF;">
                <div style="display:flex; align-items:center; gap:12px; cursor:pointer;" onclick="toggleDropdown()">
                    ...
                    <div>
                        <div style="font-weight:900; font-size:0.85rem; display:flex; align-items:center; gap:5px;">
                            ${activeWallet.handle} <span style="font-size:0.6rem; opacity:0.5;">▼</span>
                        </div>
                    </div>
                </div>
            ...
            <div id="wallet-dropdown" style="display:none; background:#F9F9F9; border-bottom:1px solid #EEE; padding:15px;">
                <div onclick="alert('Creating New .pri Wallet...')" style="padding:12px; font-weight:700; font-size:0.75rem; cursor:pointer; border-bottom:1px solid #EEE;">Create New Wallet</div>
                ...
                <div onclick="alert('Pairing Hardware Node...')" style="padding:12px; font-weight:700; font-size:0.75rem; cursor:pointer;">Connect Ledger</div>
            </div>
            ...
            <div id="wallet-content" style="flex:1; overflow-y:auto;">
...
function renderVault(ledger, wallet) {
    ...
    return `
        <div style="padding:30px; text-align:center;">
            ...
            <div style="font-size:0.6rem; font-weight:900; margin-top:40px; text-align:left; letter-spacing:2px; color:#666; text-transform:uppercase;">Anchored Collections</div>
    ...
    return `
        <div style="padding:35px;">
            <div style="background:#F9F9F9; border:1px solid #EEE; padding:20px; border-radius:20px; margin-bottom:10px;">
                <label style="font-size:0.6rem; font-weight:900; color:#666;">FROM</label>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:10px;">
                    <select id="swap-from" style="background:none; border:none; font-weight:900; font-size:1.1rem; outline:none;"><option>$PRN</option><option>$MUSD</option></select>
                    <input type="number" value="100" style="background:none; border:none; text-align:right; font-weight:900; font-size:1.1rem; width:100px; outline:none;">
                </div>
            </div>
            <div style="text-align:center; margin:-20px 0; position:relative; z-index:2;"><div style="background:#7B35D4; width:36px; height:36px; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; color:#FFF; border:4px solid #FFF;">↓</div></div>
            <div style="background:#F9F9F9; border:1px solid #EEE; padding:20px; border-radius:20px;">
                <label style="font-size:0.6rem; font-weight:900; color:#666;">TO</label>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:10px;">
                    <select id="swap-to" style="background:none; border:none; font-weight:900; font-size:1.1rem; outline:none;"><option>$MUSD</option><option>$ADA</option><option>$PRN</option></select>
                    <span style="font-weight:900; font-size:1.1rem;">250.00</span>
                </div>
            </div>
            <button onclick="alert('Settling on Chillata...')" style="width:100%; padding:20px; background:#7B35D4; color:#FFF; border:none; border-radius:100px; font-weight:900; margin-top:30px; cursor:pointer;">EXECUTE SWAP</button>
        </div>
    `;
}
    ...
    return `
        <div style="padding:40px; text-align:center;">
            <div style="background:#000; padding:25px; border-radius:25px; display:inline-block; margin-bottom:30px;">
                <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${wallet.address}" style="width:150px; border-radius:10px;">
            </div>
            <div style="background:#F9F9F9; padding:20px; border-radius:15px; border:1px solid #EEE; word-break:break-all;">
                <div style="font-size:0.75rem; font-weight:900; margin-bottom:10px;">${wallet.address}</div>
                <button onclick="navigator.clipboard.writeText('${wallet.address}');alert('Address Copied')" style="background:#1A1A1A; color:#FFF; border:none; padding:8px 20px; border-radius:100px; font-size:0.6rem; font-weight:900; cursor:pointer;">COPY ADDRESS</button>
            </div>
        </div>
    `;
}
...
function renderSend() {
    return `
        <div style="padding:40px;">
            ...
            <input type="number" placeholder="Amount" style="width:100%; padding:18px; border-radius:15px; border:1px solid #EEE; outline:none; font-weight:700;">
            <button onclick="alert('Atomic Send Confirmed')" style="width:100%; padding:20px; background:#1A1A1A; color:#FFF; border:none; border-radius:100px; font-weight:900; margin-top:30px; cursor:pointer;">CONFIRM SEND</button>
        </div>
    `;
}
    ...
    return `
        <div style="padding:25px; display:grid; grid-template-columns:1fr 1fr; gap:15px;">
            ${dapps.map(d => `<div onclick="window.open('${d.u}')" style="background:#F9F9F9; border:1px solid #EEE; padding:25px; border-radius:20px; text-align:center; cursor:pointer;">
                ...
                <div style="font-weight:900; font-size:0.7rem; letter-spacing:1px;">${d.n}</div>
            </div>`).join('')}
        </div>
    `;
}
