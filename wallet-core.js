// Priscion MUSE Wallet Core v9.0.0
// THE SOVEREIGN OS: Lynx (Voice/Attach) | Multi-Wallet (Auth Logic) | Embedded dApps | Live Ledger
            ...
            <div onclick="openWalletAuth()" style="background:${theme.surface}; border:1px solid ${theme.border}; padding:15px 20px; border-radius:20px; margin-bottom:25px; display:flex; justify-content:space-between; align-items:center; cursor:pointer;">
                <div style="display:flex; align-items:center; gap:12px;">
                    ...
                    <div>
                        ...
                        <div style="font-size:0.55rem; color:${theme.muted}; font-weight:700;">Active Identity</div>
                    </div>
                </div>
            ...
            <div style="margin-bottom:20px;">
                <div style="display:flex; gap:20px; border-bottom:1px solid ${theme.border}; padding-bottom:12px; overflow-x:auto; scrollbar-width:none;">
                    ${['vault', 'swap', 'send', 'receive', 'dapps', 'lynx'].map(tab => `
                        <button onclick="switchTab('${tab}')" style="background:none; border:none; font-weight:900; font-size:0.6rem; letter-spacing:2px; color:${currentTab===tab?theme.accent:theme.muted}; cursor:pointer; text-transform:uppercase; white-space:nowrap; border-bottom: ${currentTab===tab?'2px solid '+theme.accent:'2px solid transparent'}; padding-bottom:10px; margin-bottom:-12px;">${tab}</button>
                    `).join('')}
                </div>
            </div>
...
function renderAuth(theme) {
    return `
        <div style="display:grid; gap:12px; padding-top:10px;">
            <button class="auth-node" style="background:${theme.surface}; border:1px solid ${theme.border}; padding:25px; border-radius:20px; text-align:left; cursor:pointer; color:${theme.text};">
                <div style="font-weight:900; font-size:0.85rem;">CREATE NEW WALLET</div>
                <div style="font-size:0.6rem; color:${theme.muted}; margin-top:5px;">Generate a new .pri identity node.</div>
            </button>
            <button class="auth-node" style="background:${theme.surface}; border:1px solid ${theme.border}; padding:25px; border-radius:20px; text-align:left; cursor:pointer; color:${theme.text};">
                <div style="font-weight:900; font-size:0.85rem;">RESTORE WALLET</div>
                <div style="font-size:0.6rem; color:${theme.muted}; margin-top:5px;">Import using recovery seed phrase.</div>
            </button>
            <button class="auth-node" style="background:${theme.surface}; border:1px solid ${theme.border}; padding:25px; border-radius:20px; text-align:left; cursor:pointer; color:${theme.text};">
                <div style="font-weight:900; font-size:0.85rem;">CONNECT LEDGER</div>
                <div style="font-size:0.6rem; color:${theme.muted}; margin-top:5px;">Pair with hardware security node.</div>
            </button>
        </div>
    `;
}
        ...
        <div style="display:grid; gap:10px; margin-bottom:20px;">
            ${assets.map(a => `
                <div style="background:${theme.bg}; border:1px solid ${theme.border}; padding:18px; border-radius:18px; display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <span style="font-size:0.5rem; color:${theme.accent}; font-weight:900; letter-spacing:1px;">ANCHORED</span>
                </div>
            `).join('')}
        </div>
    `;
    ...
    return `
        <div style="display:flex; flex-direction:column; height:100%;">
            <div style="flex:1; display:flex; flex-direction:column; justify-content:center; align-items:center; opacity:0.5;">
                ...
                <div style="font-size:0.6rem; margin-top:5px;">End-to-End Jello Encryption</div>
            </div>
            <div style="border-top:1px solid ${theme.border}; padding-top:20px;">
                <div style="display:flex; gap:15px; margin-bottom:15px;">
                    <div title="Attach Vector" style="cursor:pointer; color:${theme.muted};">${icons.clip}</div>
                    <div onclick="startVoice()" title="Record Vector" style="cursor:pointer; color:${theme.muted};" id="mic-node">${icons.mic}</div>
                    <div id="voice-status" style="font-size:0.6rem; font-weight:900; color:${theme.accent}; display:none;">RECORDING...</div>
                </div>
                <div style="background:${theme.surface}; border:1px solid ${theme.border}; border-radius:100px; padding:12px 20px; display:flex; gap:12px; align-items:center;">
                    <input type="text" placeholder="Message Architect..." style="flex:1; background:none; border:none; color:${theme.text}; font-size:0.8rem; outline:none;">
                    <div style="color:${theme.accent}; cursor:pointer;">${icons.send}</div>
                </div>
            </div>
        </div>
    `;
...
function renderDapps(theme) {
    const dapps = [{n:'LEGGO', i:'🌐', d:'OS Hub', u:'leggo.html'}, {n:'PULSE', i:'📈', d:'Explorer', u:'pulse/'}, {n:'PEANUTS', i:'📊', d:'Analytics', u:'peanuts/'}];
    return `
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; padding-top:10px;">
            ${dapps.map(d => `
                <div onclick="embedDapp('${d.u}')" style="background:${theme.surface}; border:1px solid ${theme.border}; padding:20px; border-radius:20px; text-align:center; cursor:pointer;">
                    <div style="font-size:1.5rem; margin-bottom:10px;">${d.i}</div>
...
function startVoice() {
    const node = document.getElementById('mic-node');
    ...
    setTimeout(() => { 
        ...
        alert("Voice Node: Signal captured. Ready for ledger attachment.");
    }, 3000);
}
